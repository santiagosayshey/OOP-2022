1-1: function-1-1.cpp main-1-1.cpp
	g++ function-1-1.cpp main-1-1.cpp -o 1-1.out
	./1-1.out

1-2: function-1-2.cpp main-1-2.cpp
	g++ function-1-2.cpp main-1-2.cpp -o 1-2.out
	./1-2.out

1-3: function-1-3.cpp main-1-3.cpp
	g++ function-1-3.cpp main-1-3.cpp -o 1-3.out
	./1-3.out

1-4: function-1-4.cpp main-1-4.cpp
	g++ function-1-4.cpp main-1-4.cpp -o 1-4.out
	./1-4.out

1-5: function-1-5.cpp main-1-5.cpp
	g++ function-1-5.cpp main-1-5.cpp -o 1-5.out
	./1-5.out

2-1: function-2-1.cpp main-2-1.cpp
	g++ function-2-1.cpp main-2-1.cpp -o 2-1.out
	./2-1.out

2-2: function-2-2.cpp main-2-2.cpp
	g++ function-2-2.cpp main-2-2.cpp -o 2-2.out
	./2-2.out

2-3: function-2-3.cpp main-2-3.cpp
	g++ function-2-3.cpp main-2-3.cpp -o 2-3.out
	./2-3.out

2-4: function-2-4.cpp main-2-4.cpp
	g++ function-2-4.cpp main-2-4.cpp -o 2-4.out
	./2-4.out

2-5: function-2-5.cpp main-2-5.cpp
	g++ function-2-5.cpp main-2-5.cpp -o 2-5.out
	./2-5.out

3-1: function-3-1.cpp main-3-1.cpp
	g++ function-3-1.cpp main-3-1.cpp -o 3-1.out
	./3-1.out

3-2: function-3-2.cpp main-3-2.cpp
	g++ function-3-2.cpp main-3-2.cpp -o 3-2.out
	./3-2.out

3-3: function-3-3.cpp main-3-3.cpp
	g++ function-3-3.cpp main-3-3.cpp -o 3-3.out
	./3-3.out

3-4: function-3-4.cpp main-3-4.cpp
	g++ function-3-4.cpp main-3-4.cpp -o 3-4.out
	./3-4.out

3-5: function-3-5.cpp main-3-5.cpp
	g++ function-3-5.cpp main-3-5.cpp -o 3-5.out
	./3-5.out

4-1: function-4-1.cpp main-4-1.cpp
	g++ function-4-1.cpp main-4-1.cpp -o 4-1.out
	./4-1.out

4-2: function-4-2.cpp main-4-2.cpp
	g++ function-4-2.cpp main-4-2.cpp -o 4-2.out
	./4-2.out

4-3: function-4-3.cpp main-4-3.cpp
	g++ function-4-3.cpp main-4-3.cpp -o 4-3.out
	./4-3.out

4-4: function-4-4.cpp main-4-4.cpp
	g++ function-4-4.cpp main-4-4.cpp -o 4-4.out
	./4-4.out

4-5: function-4-5.cpp main-4-5.cpp
	g++ function-4-5.cpp main-4-5.cpp -o 4-5.out
	./4-5.out

main: main.cpp hunter.cpp animal.cpp vegie.cpp zoo.cpp sort_by_animalID.cpp sort_by_name.cpp
	g++ main.cpp hunter.cpp animal.cpp vegie.cpp zoo.cpp sort_by_animalID.cpp sort_by_name.cpp -o main.out
	./main.out

	